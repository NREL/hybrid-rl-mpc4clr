...
class OPFPyomo():       # PyomoOPFModel
    ...
    def __init__(self):
        ...
        self.data_path = os.path.dirname(self.current_dir)+"/gym_opf_env/opf_envs/envs/data"
    ...
    def build_load_restoration_model(self, control_horizon, fuel_reserve, soc_reserve, prior_active_restored_load, \
        ...
        def objective_fun(m):
            ...
            #    + sum(self.load_priority[b]*m.rlq[b,t]*self.delta_t for b in m.bus_set for t in m.time_set) \
            #        - sum(self.load_priority[b]*m.mup[b,t]*self.psi*self.delta_t for b in m.bus_set for t in m.time_set) \
            #            - sum(self.load_priority[b]*m.muq[b,t]*self.psi*self.delta_t for b in m.bus_set for t in m.time_set) \
...
##############################################################################################################
########################################### Load Restoration Model #2 ########################################
###        Compared to the previous model, this model works with one reserve input (system_wide_reserve)   ###
##############################################################################################################
...
class OPFPyomo2():      
    ...
    def __init__(self):
        ...
        self.Vbase = 4160
        self.Sbase = (self.Vbase**2)/self.Zbase
        self.Cbase = 800            # Storage rated capacity in kWh
        self.w_to_kW = 1000
    ...
    def build_load_restoration_model(self, control_horizon, system_wide_reserve, prior_active_restored_load, \
                                           prior_reactive_restored_load, mt_remaining_fuel, es_prior_soc, \
                                           wind_power_forecast, solar_power_forecast):
        """
        Build the load restoration model.
        ...
        m.system_wide_reserve = pyo.Param(within=pyo.NonNegativeReals, initialize=system_wide_reserve, mutable=True)
        ...
        # 14. System-wide end-of-horizon reserve requirement constraint

        m.system_wide_reserve_constraint = pyo.Constraint()
        
        m.system_wide_reserve_constraint = \
            (0, sum((m.mt_remaining_fuel[g] - sum(m.gp[g,t]*self.delta_t*self.Sbase/self.w_to_kW for t in m.time_set)) for g in m.gen_set) + \
                sum(((m.ssoc[s,m.time_set.last()] - m.ssoc_min[s])*self.Cbase) for s in m.stor_set) - m.system_wide_reserve, None)
...
##############################################################################################################
########################################### Load Restoration Model #3 ########################################
### This model is similar with the 1st model but the RL environment does not need to build the Pyomo model ###
### i.e., the model is built once here and then only required paramters can be updated from the RL env.    ###
##############################################################################################################
...
class OPFPyomo3():       
    ...
    def __init__(self):
        ...
        # Initialize the Pyomo lr model

        control_horizon = 72     # 6h        
        fuel_reserve = 0         # in kWh
        soc_reserve = 0          # in %
        num_of_bus = 13
        prior_active_restored_load = [0] * num_of_bus 
        prior_reactive_restored_load = [0] * num_of_bus 
        mt_remaining_fuel = 1000        # in kWh
        es_prior_soc = 100              # in %
        wind_power_forecast = [0]*control_horizon
        solar_power_forecast = [0]*control_horizon

        self.model = self.build_load_restoration_model(control_horizon, fuel_reserve, soc_reserve, prior_active_restored_load, \
                                                       prior_reactive_restored_load, mt_remaining_fuel, es_prior_soc, \
                                                       wind_power_forecast, solar_power_forecast)
    ...
    def solve_model(self, solver, tee, solver_options):
        """
        Solve the load restoration optimization model.
        """
    ...
    def compute_solution(self, solver, solver_tee=False, solver_options=None):
        """
        Compute the solution of the optimization model.
        """
        
        opf_converged = self.solve_model(solver, solver_tee, solver_options)

        if opf_converged:
            
            # Microturbine
            mt_p = {mt: [pyo.value(self.model.gp[mt,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for mt in self.model.gen_set}
            mt_q = {mt: [pyo.value(self.model.gq[mt,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for mt in self.model.gen_set}

            # Wind
            wt_p = {wt: [pyo.value(self.model.wdp[wt,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for wt in self.model.wind_set}
            wt_pc = {wt: [pyo.value(self.model.wdpc[wt,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for wt in self.model.wind_set}
            wt_q = {wt: [pyo.value(self.model.wdq[wt,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for wt in self.model.wind_set}

            # PV
            pv_p = {pv: [pyo.value(self.model.pvp[pv,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for pv in self.model.pv_set}
            pv_pc = {pv: [pyo.value(self.model.pvpc[pv,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for pv in self.model.pv_set}
            pv_q = {pv: [pyo.value(self.model.pvq[pv,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for pv in self.model.pv_set}

            # Storage            
            st_p = {st: [(pyo.value(self.model.spd[st,t]) - pyo.value(self.model.spc[st,t]))*self.Sbase/self.w_to_kW \
                for t in self.model.time_set] for st in self.model.stor_set}
            st_q = {st: [(pyo.value(self.model.sqd[st,t]) - pyo.value(self.model.sqc[st,t]))*self.Sbase/self.w_to_kW \
                for t in self.model.time_set] for st in self.model.stor_set}
            st_soc = {st: [pyo.value(self.model.ssoc[st,t])*100 for t in self.model.time_set] for st in self.model.stor_set}

            # Load
            ld_p = {ld: [pyo.value(self.model.rlp[ld,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for ld in self.model.load_set}
            ld_q = {ld: [pyo.value(self.model.rlq[ld,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for ld in self.model.load_set}

            # Bus voltages
            voltages = {b: [pyo.value(self.model.v[b,t]) for t in self.model.time_set] for b in self.model.bus_set}

            # Line flows
            flows_p = {b: [pyo.value(self.model.fp[b,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for b in self.model.bus_set}
            flows_q = {b: [pyo.value(self.model.fq[b,t])*self.Sbase/self.w_to_kW for t in self.model.time_set] for b in self.model.bus_set}
    ...
    def update_load_restoration_model_parameters(self, fuel_reserve, soc_reserve, active_power_restored, reactive_power_restored, 
                                                 mt_remaining_fuel, es_soc, wind_power_forecast, solar_power_forecast):
        """
        Update the lr model params.
        """
        
        m = self.model

        for g in m.gen_set:
            m.fuel_reserve[g]._value = fuel_reserve
            m.mt_remaining_fuel[g]._value = mt_remaining_fuel

        for w in m.wind_set:
            for t in m.time_set:
                m.wind_upperbound[w,t]._value = wind_power_forecast[t]

        for p in m.pv_set:
            for t in m.time_set:
                m.pv_upperbound[p,t]._value = solar_power_forecast[t]

        for s in m.stor_set:
            m.soc_reserve[s]._value = soc_reserve/100
            m.initial_soc[s]._value = es_soc/100

        for l in m.load_set:
            m.prior_restored_activepower[l]._value = active_power_restored[l]
            m.prior_restored_reactivepower[l]._value = reactive_power_restored[l]


##############################################################################################################
########################################### Load Restoration Model #4 ########################################
###                                 Similar to the 1st model except that this model                        ###
###                             relaxes the hard constraints associated with end-of-horizon reserves       ###
##############################################################################################################
...
class OPFPyomo4(): 
    ...
    def __init__(self):
        ...
        self.alpha =  0.2                # $/kWh -- penalty for wind power curtailment
        self.beta = 0.2                  # $/kWh -- penalty for PV power curtailment
        self.psi = 100                   # $/kWh -- penalty (to relax load restoration)
        self.phi = 1.5                   # $/kWh -- penalty (to relax fuel and SOC reserves)
    ...
    def build_load_restoration_model(self, control_horizon, fuel_reserve, soc_reserve, prior_active_restored_load, \
        ...
        m.gtau = pyo.Var(m.bus_set, within=pyo.NonNegativeReals)              # slack variable to relax generator fuel reserve
        m.stau = pyo.Var(m.bus_set, within=pyo.NonNegativeReals)              # slack variable to relax storage soc reserve
        ...
        def objective_fun(m):
            ...
            # w/o reactive load
            return sum(self.load_priority[b]*m.rlp[b,t]*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                - sum(self.load_priority[b]*m.mup[b,t]*self.psi*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                    - sum(m.wdpc[b,t]*self.alpha*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                        - sum(m.pvpc[b,t]*self.beta*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                                - sum((m.gtau[b]+m.stau[b]*self.Cbase)*self.phi for b in m.bus_set)    
        ...
        m.gp_fuel_usage_constraint = pyo.Constraint(m.gen_set)
        m.gq_fuel_usage_constraint = pyo.Constraint(m.gen_set) 
        m.gen_fuel_reserve_relaxation = pyo.Constraint(m.gen_set)
        ...
        # Relax fuel reserve rule
        for g in m.gen_set:
            m.gen_fuel_reserve_relaxation[g] = \
                (0, m.gtau[g] - (m.fuel_reserve[g] - \
                    (m.mt_remaining_fuel[g] - sum(m.gp[g,t]*self.delta_t*self.Sbase/self.w_to_kW for t in m.time_set))), None)
        ...
        m.ssoc_upperbound_constraint = pyo.Constraint(m.stor_set, m.time_set)        
        m.storage_soc_dynamics = pyo.Constraint(m.stor_set, m.time_set)        
        m.storage_soc_reserve_relaxation = pyo.Constraint(m.stor_set)     
        ...
        # Relax SOC reserve rule
        for s in m.stor_set:
            m.storage_soc_reserve_relaxation[s] = \
                (0 , m.stau[s] - (m.soc_reserve[s] - m.ssoc[s,m.time_set.last()])*self.Cbase, None)
        ...
        m.gp_non_gen_bus_constraint = pyo.Constraint((m.bus_set - m.gen_set), m.time_set)
        m.gq_non_gen_bus_constraint = pyo.Constraint((m.bus_set - m.gen_set), m.time_set)
        m.gtau_non_gen_bus_constraint = pyo.Constraint((m.bus_set - m.gen_set))

        for b in (m.bus_set - m.gen_set):
            m.gtau_non_gen_bus_constraint[b] = (m.gtau[b] == 0.0)
            for t in m.time_set:
                m.gp_non_gen_bus_constraint[b,t] = (m.gp[b,t] == 0.0)
                m.gq_non_gen_bus_constraint[b,t] = (m.gq[b,t] == 0.0)
        ...
        m.sc_non_storage_bus_constraint = pyo.Constraint((m.bus_set - m.stor_set), m.time_set)
        m.sd_non_storage_bus_constraint = pyo.Constraint((m.bus_set - m.stor_set), m.time_set)
        m.stau_non_storage_bus_constraint = pyo.Constraint((m.bus_set - m.stor_set))
      
        for b in (m.bus_set - m.stor_set):
            m.stau_non_storage_bus_constraint[b] = (m.stau[b] == 0)
            for t in m.time_set:
                m.spc_non_storage_bus_constraint[b,t] = (m.spc[b,t] == 0)
                m.spd_non_storage_bus_constraint[b,t] = (m.spd[b,t] == 0)                
                m.sqc_non_storage_bus_constraint[b,t] = (m.sqc[b,t] == 0)
    ...
    def update_load_restoration_model(self, control_horizon, fuel_reserve, soc_reserve, prior_active_restored_load, \
                                            prior_reactive_restored_load, mt_remaining_fuel, es_prior_soc, \
                                            wind_power_forecast, solar_power_forecast, \
                                            mt_terminal_state_power, es_terminal_state_power):
        """
        Update the load restoration model - applies during the terminal state in the RL Env.
        """

        m = self.build_load_restoration_model(control_horizon, fuel_reserve, soc_reserve, prior_active_restored_load, \
                                              prior_reactive_restored_load, mt_remaining_fuel, es_prior_soc, \
                                              wind_power_forecast, solar_power_forecast)

        ## Fix all the variables we care.

        # MT active power
        m.gp.fix(mt_terminal_state_power*self.w_to_kW/self.Sbase)

        # ES active power
        m.spc.fix(0)
        m.spd.fix(es_terminal_state_power*self.w_to_kW/self.Sbase)       
        m.sc.fix(0)
        m.sd.fix(1) 
...
##############################################################################################################
########################################### Load Restoration Model #5 ########################################
###                                 Similar to the 1st model except that this model                        ###
###                             relaxes the hard constraints associated with end-of-horizon reserves       ###
###             besides, end-of-horizon reserves are explicitly related to start-of-horizon reserves       ###
##############################################################################################################
...
class OPFPyomo5(): 
    ...
    def build_load_restoration_model(self, control_horizon, fuel_reserve_factor, soc_reserve_factor, prior_active_restored_load, \
                                           prior_reactive_restored_load, mt_remaining_fuel, es_prior_soc, \
                                           wind_power_forecast, solar_power_forecast):
        """
        Build the load restoration model.
        ...
        m.gq_max = pyo.Param(m.gen_set, within=pyo.NonNegativeReals, \
            initialize={self.Generator[g]['bus_idx']-1 : self.Generator[g]['g_Q_max'] for g in self.Generator.keys()})
        m.gen_pf = pyo.Param(m.gen_set, within=pyo.NonNegativeReals, initialize=0.75)
        m.fuel_reserve_factor = pyo.Param(m.gen_set, mutable=True, within=pyo.UnitInterval, initialize=fuel_reserve_factor)
        m.mt_remaining_fuel = pyo.Param(m.gen_set, mutable=True, within=pyo.NonNegativeReals, initialize=mt_remaining_fuel)
        ...
        m.ssoc_max = pyo.Param(m.stor_set, within=pyo.NonNegativeReals, \
            initialize={self.Storage[s]['bus_idx']-1 : self.Storage[s]['s_SOC_max'] for s in self.Storage.keys()})
        m.initial_soc = pyo.Param(m.stor_set, mutable=True, within=pyo.NonNegativeReals, initialize=es_prior_soc/100)
        m.soc_reserve_factor = pyo.Param(m.stor_set, mutable=True, within=pyo.UnitInterval, initialize=soc_reserve_factor)
        m.charging_eff = pyo.Param(m.stor_set, within=pyo.NonNegativeReals, \
            initialize={self.Storage[s]['bus_idx']-1 : self.Storage[s]['s_eff_char'] for s in self.Storage.keys()})
        m.discharging_eff = pyo.Param(m.stor_set, within=pyo.NonNegativeReals, \
            initialize={self.Storage[s]['bus_idx']-1 : self.Storage[s]['s_eff_dischar'] for s in self.Storage.keys()})
        ...
        def objective_fun(m):
            ...
            return sum(self.load_priority[b]*m.rlp[b,t]*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                - sum(self.load_priority[b]*m.mup[b,t]*self.psi*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                    - sum(m.wdpc[b,t]*self.alpha*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                        - sum(m.pvpc[b,t]*self.beta*self.delta_t*self.Sbase/self.w_to_kW for b in m.bus_set for t in m.time_set) \
                                - sum((m.stau[b]*self.Cbase)*self.phi for b in m.bus_set)    # m.gtau[b]+                      
        ...
        #for g in m.gen_set:
        #    m.gen_fuel_reserve_relaxation[g] = \
        #        (0, m.gtau[g] - (m.fuel_reserve_factor[g]*m.mt_remaining_fuel[g] - \
        #            (m.mt_remaining_fuel[g] - sum(m.gp[g,t]*self.delta_t*self.Sbase/self.w_to_kW for t in m.time_set))), None)
        ...
        for s in m.stor_set:
            for t in m.time_set:
                ...
                if t == m.time_set.last():
                    m.storage_soc_reserve_relaxation[s] = \
                        (0 , m.stau[s] - (m.soc_reserve_factor[s]*m.initial_soc[s] - m.ssoc[s,t]), None)               
